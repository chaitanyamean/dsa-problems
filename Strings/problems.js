...
function isAlpha(A) {
  for (let i = 0; i < A.length; i++) {
    ...
    if (
      (A[i].charCodeAt(0) >= 65 && A[i].charCodeAt(0) <= 90) ||
...
// function sortedLexograpically(A) {
//     let B = A.split('')
//     B.sort()
//     console.log(B.join(''))
...
String.prototype.myReverstr = function (...args) {
  let A = args[0];
  let n = A.length;
  let newStr = "";
  for (let i = n - 1; i >= 0; i--) {
...
let bb = a.myReverstr.call(this, a);
...
const add9 = 9;
const result = sum.bind(this, add9);
...
let total = 0
  ...
  for (let i = 0; i < A.length; i++) {
    ...
    for (let j = i + 1; j < A.length; j++) {
      console.log("A[j]", A[j]);
      if (A[i].charCodeAt(0) > A[j].charCodeAt(0)) {
        cnt++;
      }
    }
    let facVal = A.length - (i + 1)
    let facSum = 1
    for(let k=1; k<=facVal;k++) {
        facSum = (facSum * k) % 1000003
    }
    total += (facSum * cnt) % 1000003;
    total = total % 1000003;
  }
